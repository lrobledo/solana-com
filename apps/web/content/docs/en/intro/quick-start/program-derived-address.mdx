...
Learn how to build a basic program that allows users to user to create, update, and delete a message.
Each message exists in an account with a deterministic address.
The address a called a [Program Derived Addresses](/docs/core/pda) (PDA)
because it is derived from the program itself.

This guide walks you through building and testing a Solana program using the
Anchor framework while demonstrating PDAs.
...
First, we will define the structure of the data to be stored on the message account.

**a.** Open the [starter code](https://beta.solpg.io/66734b7bcffcf4b13384d1ad).

**b.** Click **Import** to add the program to your Solana Playground projects.

**c.** Enter a project name where prompted.

![Import](/assets/docs/intro/quickstart/pg-import.png)

<WithMentions>

In `lib.rs`, you will see a program with the [`create`](mention:one),
[`update`](mention:two), and [`delete`](mention:three) instructions.
You will update these instructions in subsequent steps.
...
**d.** Run _shell`build`_ in the terminal to verify the program builds successfully.
...
In the next step, we will add the program instructions.

</Step>
<Step>


### Define the Message Account Type

<WithNotes>

**a.** In `lib.rs`, update the _rs`MessageAccount`_ struct with the following:
...
**b.** Build the program again by running _shell`build`_ in the terminal.

```terminal
$ build
```

This structure defines the data to be stored in the account.
In the next step we will add the program instructions.
...
[//]: # (Just checking that it can create multiple message accounts? tk)
Add the _rs`create`_ instruction that will create and initialize a new _rs`MessageAccount`_.

**a.** Replace the _rs`Create`_ struct with the code below.
This will define the accounts required for the _rs`create`_ instruction.
...
**b.** Update the `create` function with the code below.
This adds the business logic for the _rs`create`_ instruction.
...
Add the `update` instruction that will update an existing `MessageAccount` with a new message.

[//]: # (Make sure 'replace' is accurate/ok)
**a.** Replace the `Update` struct with the code below.
This specifies the accounts required by the `update` instruction.
...
[//]: # (Logic vs business logic)
**b.** Add the logic for the `update` instruction.
...
### Add the Delete Instruction

**a.** Update the _rs`Delete`_ struct with the code below.
This will add the _rs`delete`_ instruction to close the _rs`MessageAccount`_.
...
**c.** Add the logic for the `delete` instruction.
...
You have now completed the basic CRUD program.
Deploy the program by running `deploy` in the terminal.
...
The Playground wallet connects to the [devnet cluster](http://localhost:3000/docs/references/terminology#connected-cluster) by default.
Meaning this program will be deployed to the devnet cluster.

</Callout>

<Callout type="warn">

Ensure your [Playground wallet has devnet SOL](/docs/intro/quick-start#add-sol-to-your-wallet) to pay for the program deployment.
...
The starter code includes a test file, shown below.
...
**a.** Add the code below inside _ts`describe()`_, but before the _ts`it()`_ sections.
...
**b.** Run the test file verify that it runs as expected.
(In the next steps we will add tests.)
...
Update the first test with the code below
...
<WithNotes>
Update the second test with the code below:
...
### Invoke the Delete Instruction

<WithNotes>

Update the third test with the code below.
...
You are now ready to run your updated test file.
It will test the program you deployed on the devnet cluster
and logs links to SolanaFM to view the transaction details.
...
<Callout type="info">
  In this example, if you run the test again, the `create` instruction will fail.
  This is because `messageAccount` already exists and only one account can exist for a given PDA.
</Callout>
