...
Build, deploy, and test your first [program](/docs/core/programs) (smart contract)
to the Solana Blockchain using the Anchor framework.

This guide will help you become familiar with the Solana Playground

walks through a more detailed example in the Program
Derived Address (PDA) and Cross-Program Invocation (CPI) sections.

, refer to the [Programs on Solana](/docs/core/programs) page.
...
You will see a new project created with the program code in the `src/lib.rs` file.
This basic Solana program creates a new account and stores a number in it.
The program contains one instruction (`initialize`) which:
