...
Fetch accounts from the Solana network to get familiar with the account structure.
On Solana, all data exists in Accounts.
You can think of data in Solana as a public database with a single Accounts table.
Each entry in the table is an [Account](docs/core/accounts) with the same structure.

![Accounts](/assets/docs/core/accounts/accounts.png)

Accounts can store state or executable programs.
Each account has a public key called the address that serves as its unique ID.
The address points to its corresponding on-chain data.

- **State**: Persistent data that is meant to be read.
  For example, information about tokens, user data, or other data defined within a program.
...
The separation of code and state is a key feature of Solana's Account Model.
For more details, see the [Account Model](/docs/core/accounts) page.

## Fetch Wallet Account

This example demonstrates how to:

1. Generate a new key pair (public/private key pair).
...
by the System Program. All "wallet" accounts are System Program owned
accounts that hold SOL and can sign transactions.

[//]: # (^ ?tk)

[//]: # (Why is this different from the playground?)
...
(one of Solana’s built-in programs). Wallet accounts are primarily used
