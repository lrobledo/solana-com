...
Cross Program Invocation (CPI) allows one Solana program to invoke another.
To demonstrate this, we will create a new program and use it to
invoke the program from the [previous section]().
When making [CPIs]((/docs/core/cpi)), programs can sign for
[Program Derived Addresses](/link to PDA).

[//]: # (Add reason why this is important)

This guide walks you through implementing a CPI in a Solana program
...
**a.** Add a pay-to-update mechanism.
To do so, we will change the _rs`Update`_ struct and `update` function.

Update the `lib.rs` file to bring items from the `system_program` module into scope.
...
Update the _rs`Update`_ struct to include a new account called
`vault_account`. This account is controlled by the program and receives SOL from a
...
In this section, we added a new account (`vault_account`) to the _rs`Update`_struct.
This account will be a program-controlled vault that receives SOL
from users when they update their messages.

To manage funds, the program creates a program-controlled account (PDA) unique to each user.

---

Key aspects of the `vault_account`:

- The address of the account comes from a PDA, which is derived using seeds
  _rs`[b"vault", user.key().as_ref()]`_
- As a PDA, it has no private key. Only the program can sign for the
...
Next, we will add CPI logic to the `update` instruction.
It will transfer 0.001 SOL from the user's account to the vault account.
...
[//]: # (Would this look better as a table?)
### !transfer_accounts

[//]: # (System Program = Official terminology?)
The _rs`Transfer`_ struct specifies the required accounts for the transfer instruction.

### !cpi_context

The _rs`CpiContext`_ struct specifies the program and accounts to be used in the Cross
Program Invocation (CPI).

### !transfer

The _rs`transfer()`_ function invokes the transfer instruction.
...
[//]: # (instruction, struct and structure. Look up terms in rust.)
The `update` instruction now includes a CPI
to invoke the System Program's `transfer` instruction.
This enables the composability of Solana programs.
...
[//]: # (Change to Term: Definition)

- The program to invoke (System Program)
- The accounts required in the CPI (defined in the _rs`Transfer`_ struct)

[//]: # (I don't like how often we're referring to CPI in this demo.)
[//]: # (Should the CPI core concept section come up above this in order?)
...
[//]: # (Inconsistent with the use of variables here.)
...
A CPI setup is similar to client-side instructions, in which
get built, where you
specify the program, accounts, and instruction data for a particular instruction
to invoke.
When the program's `update` instruction receives an invocation, it
...
Next, we will add the CPI logic in the `delete` instruction to transfer SOL from the
...
This allows a program to sign for a PDA that is derived from its program ID.

[//]: # (^^)

When the instruction is processed, the runtime verifies the signer seed.
If verified, the runtime treats that PDA account as a signer for the duration of the CPI.
