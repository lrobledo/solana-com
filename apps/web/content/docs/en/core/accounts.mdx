...
---
## Overview
On the Solana network, all information is stored in **accounts**.

You can think of the Solana network as a big table of accounts. Each row in the table is an account with its own address.
...
In order to locate an account, you must be able to call its address. The account address acts as the unique ID 
...
pair. In the Solana account model, the "key" is the account address that gives you access to the "value"—the corresponding
on-chain information in the account. 
...
Every Solana account has a [program](/docs/core/programs) designated as its owner. Only the program
that owns an account can change the account's `data` field or deduct lamports from its balance. Anyone can increase the balance.

For executable accounts, the `owner` field contains the program ID of a 
