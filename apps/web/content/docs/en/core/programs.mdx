...
Solana smart contracts are called programs.
Programs are [accounts](https://solana.com/docs/core/accounts) that contain executable binary.
The code determines how data will be read, written, or modified in other accounts—often determining
...
Solana programs are mainly written in the
...
- [Anchor](https://www.anchor-lang.com/docs): Anchor is a framework designed for fast and easy Solana
  development. It writes programs in Rust, but provides macros
...
which is typically the same account that originally [deployed the program](/docs/programs/deploying).
If the upgrade authority is revoked and set to `None`, the program can no longer be updated.
Usually, program developers revoke the upgrade authority once a program reaches the production stage.
This gives users confidence that the terms of the transaction won't change.

## Verifying programs

Verifiable builds allow users to check whether a program's on-chain code matches its public source code.
You can create a verifiable build using [Anchor's built-in support](https://www.anchor-lang.com/docs/verifiable-builds).

To check whether an existing program is verified, you may:

- Search for its program ID to find it on the [Solana Explorer](https://explorer.solana.com/address/PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY)
  (Verified programs display a green "Program Source Verified" flag.)
- Use the Ellipsis Labs [Solana Verifiable Build CLI](https://github.com/Ellipsis-Labs/solana-verifiable-build),
...
Every program is owned by a loader program.
Loaders are used to deploy, redeploy, upgrade or close programs.
They can also finalize a program and transfer program authority.
...
<Callout>
Programs deployed with the native loader, loader-v1 or loader-v2 are immutable after deployment.
</Callout>
...
This program allows users to send instruction data
in a transaction for signature and message data.
Users can receive data from a transaction
by using a special instructions sysvar.
...
The Solana cluster genesis includes a list of special programs that provide the network's core functionality.
Historically these were referred to as "native" programs and they used to be distributed together with the validator code.
See the table below for information about each of the core programs.
